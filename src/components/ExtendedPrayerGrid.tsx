...
interface ExtendedPrayerGridProps {
  prayers: Prayer[]
  fontScale: number
}

export function ExtendedPrayerGrid({ prayers, fontScale }: ExtendedPrayerGridProps) {
  return (
    <section id='extended' className='bg-white py-12 sm:py-16 lg:py-20'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <h2 className='text-3xl sm:text-4xl font-bold text-gray-900 text-center mb-12'>บทเสริม</h2>
        {prayers.length > 0 ? (
          ...
          <p className='text-center text-gray-500'>ไม่พบบทสวดที่ตรงกับคำค้นหา</p>
        )}
      </div>
    </section>
  )
